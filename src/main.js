...
const davhtsalNemeh = _str => {
    let davhtsiinHemjee;

    rarity.forEach((r) => {
        if (_str.includes(r.key)) {
            davhtsiinHemjee = r.val;
        }
    });

    return davhtsiinHemjee;
};
...
const davhraguudinZurag = async(_layer, _хувилбар) => {
    ...
    if (element) {
        ...
        saveLayer(canvas, _хувилбар);
    }
};

const createFiles = async хувилбар => {
    const layers = layersSetup(layersOrder);

    let numDupes = 0;
    for (let i = 1; i <= хувилбар; i++) {
        await layers.forEach(async(layer) => {
            await davhraguudinZurag(layer, i);
        });
        ...
        if (Exists.has(key)) {
         ...
         key
            ...
            numDupes++;
            if (numDupes > хувилбар) break;
            i--;
        } else {
            Exists.set(key, i);
            addMetadata(i);
            console.log("Хувилбаруудыг хийж байна. " + i);
        }
    }
};
...
const createMetaData = () => {
    fs.stat(`${buildDir}/${metDataFile}`, (err) => {
        if (err == null || err.code === 'ENOENT') {
            ...
            console.log('Алдаа гарлаа: ', err.code);
        }
    });
};
